...
After removing the zero variance predictors, the set has now `r ncol(trainData)` possible predictors.
...
We previously detected that there were a lot of missing values, so we drop the predictors which have more than 50% of missing values. This threshold value of 50% is somewhat arbitrary, we will review this if the model performs poorly. The set has now `r ncol(trainData)` possible predictors.
